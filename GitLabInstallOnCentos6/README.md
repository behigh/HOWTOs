...
2 Установка Ruby
-----------------
Выберите один из способов установки Ruby.
...
Дабы не устанавливать документацию при установке гемов и тем самым ускорив в разы их установку, создадим файл:
...
-------------------------
GitLab у меня крутится под пользователем **gitlab**. Создадим данного пользователя и его директорию:
...
[RVM Ruby install] Присвоим пользователю **gitlab** группу **rvm**, чтобы oн мог запускать Ruby.
...
**[OLD]** В общем у меня тут проблемы в том что сокет создается через раз. Может это связано с последним коммитом автора, так как на рабочем сервере работает такая связка и никаких проблем.
